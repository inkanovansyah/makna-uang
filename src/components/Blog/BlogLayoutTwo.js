    ...
    <>
      {displayedBlogs.map((blog, index) => {
        ...
        return (
          <article
            key={blog.id || index}
            className="group grid grid-cols-12 gap-4 items-center text-dark dark:text-light mb-6"
          >
            <Link
              href={blogUrl}
            ...
            >
              <Image
                src={imageSrc.startsWith("http") ? imageSrc : "/placeholder.png"}                alt={blog.title || "No title"}
                width={1920}
                height={1280}
